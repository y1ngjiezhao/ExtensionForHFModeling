...
#library(Xtable)
# Viewtable()
setwd("D:/University of Queensland/Data Science/Semester 1 2025/1_Extension of capstone project/PLOTS_829_LGBM")
IDF <- read_csv("8LGBM.csv")
#####################################################
IDF$ModelGroup <- ifelse(grepl("LGBM", IDF$Forecast_Methods), "LGBM")
for (i in 10:12) {
  ...
  ggsave(filename = paste0("LGBM-level-", i, ".pdf"), plot = p, device = "pdf")#width = 8, height = 6,
}
...
for (i in 10:12) {
  ...
  ggsave(filename = paste0("LGBM-level-", i, ".pdf"),
         plot = p, device = "pdf")
}
...
for (i in 10:12) {
  ...
  ggsave(filename = paste0("LGBM-level-costs-", i, ".pdf"), plot = p, device = "pdf")#width = 8, height = 6,
}
